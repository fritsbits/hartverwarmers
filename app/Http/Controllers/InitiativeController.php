    ...
    public function index(DiamantService $diamant): View
    {
        ...
        $initiatives = Initiative::query()
            ...
            ->addSelect(['latest_fiche_at' => Fiche::query()
                ->select('created_at')
                ->whereColumn('initiative_id', 'initiatives.id')
                ->published()
                ->orderByDesc('created_at')
                ->limit(1),
            ])
            ->withCount(['fiches as recent_fiches_count' => fn ($q) => $q->published()->where('created_at', '>=', $sixtyDaysAgo)])
            ->with(['fiches' => fn ($q) => $q->published()
                ->select('id', 'initiative_id', 'user_id', 'title', 'slug', 'created_at', 'icon')
        ...
        $defaultIconSvg = '<svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/></svg>';
        $iconSvgCache = [];
            ...
            ->mapWithKeys(fn ($i) => [
                $i->slug => [
                    ...
                    'fiches' => $i->fiches
                        ...
                        ->sortByDesc('created_at')
                        ->take(3)
                        ->map(function ($f) use ($i, $defaultIconSvg, &$iconSvgCache) {
                            $ficheColors = config('fiche-icons.colors');
                            $ficheColor = $ficheColors[$f->id % count($ficheColors)];

                            $iconSvg = $defaultIconSvg;
                            if ($f->icon) {
                                $iconSvg = $iconSvgCache[$f->icon] ??= view('components.lucide-icon-svg', ['icon' => $f->icon])->render();
                            }
