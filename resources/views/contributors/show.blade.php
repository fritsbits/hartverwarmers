    ...
    <div class="max-w-6xl mx-auto px-6 py-12">
        ...
        <div class="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div class="md:w-64 text-center">
                @if($contributor->avatar_path)
                    <div class="flex justify-center">
                        <img src="{{ Storage::url($contributor->avatar_path) }}" alt="{{ $contributor->name }}" class="w-32 h-32 rounded-full object-cover">
                    </div>
                @else
                    <div class="flex justify-center">
                ...
                <h1 class="text-5xl mt-4">{{ $contributor->name }}</h1>
                ...
                @if($contributor->organisation)
                    <p class="font-medium mt-2">{{ $contributor->organisation->name }}</p>
                @endif
            </div>
            ...
            <div class="flex-1">
                ...
                @if($contributor->elaborations->isNotEmpty())
                    <flux:heading size="lg" class="mb-4">Uitwerkingen</flux:heading>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($contributor->elaborations as $elaboration)
