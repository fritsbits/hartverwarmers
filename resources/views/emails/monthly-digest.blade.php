            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;width:100%">
                            ...
                            @foreach ($payload->themes as $occurrence)
                                <tr>
                                    <td style="padding:14px 16px;{{ ! $loop->last ? 'border-bottom:1px solid #EBE4DE;' : '' }}">
                                        <a href="{{ \App\Support\NewsletterLink::tracked($notifiable, \App\Support\EmailLink::to(route('themes.index', ['maand' => $occurrence->start_date->format('Y-m')]) . '#thema-' . $occurrence->theme->slug, 'monthly-digest', 'newsletter', 'themes')) }}" style="text-decoration:none;color:inherit;display:block">
                ...
                <tr>
                    <td style="padding:0 0 36px 0">
                        ...
                        <a href="{{ \App\Support\NewsletterLink::tracked($notifiable, \App\Support\EmailLink::to(route('fiches.show', [$payload->diamond->initiative, $payload->diamond]), 'monthly-digest', 'newsletter', 'diamond')) }}" style="text-decoration:none;color:inherit;display:block">
                            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" class="card" style="background:#FFFFFF;border:1px solid #EBE4DE;border-radius:10px">
                                <tr>
                                ...
                                <tr>
                                    @foreach ($row as $fiche)
                                        <td class="fiche-cell" valign="top" width="50%" style="padding:0 5px 10px 0">
                                            <a href="{{ \App\Support\NewsletterLink::tracked($notifiable, \App\Support\EmailLink::to(route('fiches.show', [$fiche->initiative, $fiche]), 'monthly-digest', 'newsletter', 'recent-fiche')) }}" style="text-decoration:none;color:inherit;display:block">
                        ...
                        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" class="card" style="background:#FDF3EE;border:1px solid #EBE4DE;border-radius:10px">
                            <tr>
                                <td style="padding:18px 20px;line-height:1.6;font-size:14px;color:#231E1A">
                                    ...
                                    <a href="{{ \App\Support\NewsletterLink::tracked($notifiable, \App\Support\EmailLink::to(url('/contact?reden=feedback'), 'monthly-digest', 'newsletter', 'feedback')) }}" style="color:#E8764B;font-weight:600;text-decoration:none;white-space:nowrap">Geef feedback &rarr;</a>
                                </td>
                            </tr>
                        </table>
                    </td>
                ...
                <tr>
                    <td style="padding:24px 0 0 0;border-top:1px solid #EBE4DE;text-align:center;color:#756C65;font-size:12px;line-height:1.6">
                        <p style="margin:0 0 8px 0;color:#231E1A;font-size:14px">Heb je zelf iets moois op poten gezet?<br>
                            <a href="{{ \App\Support\NewsletterLink::tracked($notifiable, \App\Support\EmailLink::to(url('/fiches/nieuw'), 'monthly-digest', 'newsletter', 'create-fiche')) }}" style="color:#E8764B;font-weight:600;text-decoration:none">Deel jouw fiche &rarr;</a>
                        </p>
