@props(['fiche', 'compact' => false, 'hideDiamond' => false])
...
<flux:card class="fiche-card !p-0 overflow-hidden border border-[var(--color-border-light)] hover:border-[var(--color-border-hover)] hover:shadow-card-hover hover:-translate-y-0.5 active:scale-[0.99] transition-[border-color,box-shadow,transform] duration-200">
    ...
    <div class="fiche-card-body">
        ...
        @endif
        <flux:heading size="lg" class="font-body font-semibold text-[var(--color-text-primary)]">{{ $fiche->title }}</flux:heading>

        @if($fiche->description)
            <flux:text class="mt-1 line-clamp-2 text-sm">
                {{ Str::limit(strip_tags($fiche->description), 100) }}
            </flux:text>
        @endif

        <div class="flex items-center gap-2 mt-3 pt-3 border-t border-[var(--color-border-light)] text-sm text-[var(--color-text-secondary)]">
            @if($fiche->user)
                <div class="flex items-center gap-2 min-w-0 flex-1">
                    <x-user-avatar :user="$fiche->user" size="xs" />
                    <span class="truncate">{{ $fiche->user->full_name }}</span>
                </div>
            @endif
