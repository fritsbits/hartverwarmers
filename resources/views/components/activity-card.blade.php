@props(['activity'])

<flux:card class="overflow-hidden hover:shadow-md transition-shadow">
    @if($activity->fiche['image'] ?? null)
        <div class="-mx-6 -mt-6 mb-4">
            <img src="{{ $activity->fiche['image'] }}" alt="{{ $activity->title }}" class="w-full aspect-[16/10] object-cover">
        </div>
    @else
        <div class="-mx-6 -mt-6 mb-4 bg-zinc-100 dark:bg-zinc-800 aspect-[16/10] flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-zinc-300 dark:text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
        </div>
    @endif

    <flux:heading size="lg">{{ $activity->title }}</flux:heading>

    @if($activity->description)
        <flux:text class="mt-2 line-clamp-2">
            {{ Str::limit(strip_tags($activity->description), 120) }}
        </flux:text>
    @endif

    @if($activity->interests->isNotEmpty())
        <div class="flex flex-wrap gap-1 mt-3">
            @foreach($activity->interests->take(3) as $interest)
                <flux:badge size="sm" color="zinc">{{ $interest->name }}</flux:badge>
            @endforeach
        </div>
    @endif

    <div class="flex justify-end mt-4">
        <a href="{{ route('activities.show', $activity) }}" class="cta-link">
            Bekijk activiteit
        </a>
    </div>
</flux:card>
