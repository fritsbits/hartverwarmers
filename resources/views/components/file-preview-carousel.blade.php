...
@if($total > 0)
    <div
        ...
        class="focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:ring-offset-2"
        data-carousel
    >
        {{-- Carousel viewport --}}
        <div class="relative bg-[var(--color-bg-cream)] rounded-2xl aspect-[4/3] overflow-hidden">
                    ...
                    @foreach($slides as $index => $slide)
                        <div class="w-full shrink-0 h-full flex items-center justify-center p-8 sm:p-12">
                            ...
                            @else
                                ...
                                <div class="bg-white rounded-sm w-full max-w-[280px] aspect-[3/4] p-6 sm:p-8 flex flex-col" style="box-shadow: 0 4px 20px -4px rgba(120, 90, 60, 0.15), 0 1px 4px rgba(120, 90, 60, 0.08); border: 1px solid rgba(120, 90, 60, 0.1);" data-skeleton-card>
            ...
            <button
                ...
                class="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-[var(--color-bg-accent-light)] hover:shadow-lg hover:scale-105 active:scale-95 transition-all cursor-pointer border border-[var(--color-border-light)]"
                aria-label="Vorige"
            >
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[var(--color-text-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <button
                ...
                x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0"
                class="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-[var(--color-bg-accent-light)] hover:shadow-lg hover:scale-105 active:scale-95 transition-all cursor-pointer border border-[var(--color-border-light)]"
                aria-label="Volgende"
            >
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[var(--color-text-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                ...
                <template x-for="i in total" :key="i">
                    ...
                    >
                        <span
                            ...
                            :class="current === i - 1 ? 'w-5 h-2 bg-[var(--color-primary)]' : 'w-2 h-2 bg-zinc-300 group-hover:bg-zinc-400'"
                        ></span>
                    </button>
                </template>
            </div>
