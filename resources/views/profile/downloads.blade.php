<x-sidebar-layout title="Downloads" section-label="Profiel" description="Fiches die je hebt gedownload.">
    @if($fiches->isEmpty())
        <div class="text-center py-12">
            <flux:icon name="arrow-down-tray" class="size-16 mx-auto text-[var(--color-border-light)] mb-4" />
            <flux:text class="text-[var(--color-text-secondary)] mb-4">Nog geen downloads</flux:text>
    ...
    @else
        <p class="text-sm text-[var(--color-text-secondary)] mb-6">
            <strong class="text-[var(--color-text-primary)]">{{ $fiches->count() }}</strong> {{ $fiches->count() === 1 ? 'download' : 'downloads' }}
        </p>
